-- Falurian BLU GearSwap v2.0.0
-- 2026-08-16 core rebuild on the current Falurian RDM v2.61 / WAR v3.3.8 foundation.
...
local BLU_RELEASE_VERSION = '2.0.0'
local BLU_RELEASE_DATE = '2026-08-16' 
...
local DW_TIER_VALUE = {[1]=10,[2]=15,[3]=25,[4]=30,[5]=35,[6]=40}
...
local function refresh_trait_cache(force)
...
local function invalidate_haste_cache()
...
local function update_native_haste_dw(force)
...
function update_hud(force)
    ...
    local key=table.concat({selected,effective,live,tostring(weapon_ok),state.OffenseMode.value,state.HybridMode.value,
        ...
        blu_hud_config.layout,tostring(gi_alive)},'|')
    ...
    if section_on('magic') then
        rows[#rows+1]=hrow('MAGIC',hfield('Casting',state.CastingMode.value)..'  '..hfield('FC',tostring(trait_cache.value.FastCast)..' trait + '..tostring(fc_gear)..' gear','cyan')..'  '..hfield('Burst',burst_hud_status(),'yellow')..'  '..hfield('Set spells',set_count))
...
local function report_keybinds(startup_summary)
    ...
    chat(158,' Ctrl+F1 Melee: Normal > MidAcc > HighAcc > PDL | Ctrl+F2 Melee DT: Off/On')
    ...
    if not startup_summary then
        ...
        chat(158,' Info: traits | dwinfo | fcinfo | hasteinfo | burstinfo | unbridledinfo | thinfo | moveinfo | spellinfo <name> | policy | version')
    else
...
function user_setup()
    ...
    state.OffenseMode:options('Normal','MidAcc','HighAcc','PDL')
...
function job_post_midcast(spell, action, spellMap, eventArgs)
    ...
    if spell.skill == 'Blue Magic' then
        ...
        if family == 'Healing' and state.WeaponLock and not state.WeaponLock.value then
...
local function report_policy()
    ...
    chat(158, string.format('[BLU Policy] %d exact spell routes across %d families; duplicates=%d.',
        total, #family_names, #BLUE_POLICY_DUPLICATES))
