...
local WAR_RELEASE_VERSION = '3.3.8'
local WAR_RELEASE_DATE = '2026-08-16'
...
local WAR_FENCER_GEAR_RANK={['War. Beads +2']=1,['Boii Cuisses +1']=2}
...
local apply_fishing_slot_policy
...
local function track_war_event(id)
    if id ~= nil then WAR_RUNTIME.event_ids[#WAR_RUNTIME.event_ids+1]=id end
    return id
end

local function unregister_war_events()
    if windower and type(windower.unregister_event)=='function' then
        for _,id in ipairs(WAR_RUNTIME.event_ids) do pcall(windower.unregister_event,id) end
    end
    WAR_RUNTIME.event_ids={}
end
...
local FISHING_LOCK_SLOTS={'main','sub','range','ammo','head','neck','ear1','ear2','body','hands','back','waist','legs','feet'}
...
local function protected_ring_active(slot)
    return releasing[slot]~=true
        and NO_SWAP_GEAR[current_ring_name(slot)]==true
end


apply_fishing_slot_policy=function()
...
local function queue_weapon_apply_command(key,generation,stage,delay)
    local token=WAR_RUNTIME.token
    coroutine.schedule(function()
        if WAR_RUNTIME.unloading or token~=WAR_RUNTIME.token then return end
...
local function run_weapon_apply_stage(key,generation,stage)
    if WAR_RUNTIME.unloading or generation~=weapon_apply_state.generation then return end
    ...
    if stage=='prepare' then
        ...
        -- moving between 2H+grip, 1H+shield/offhand, and H2H because FFXI may
...
local function begin_weapon_profile_apply(key,force)
    ...
    send_command('gs c _weaponapply '..WAR_RUNTIME.token..' '..tostring(generation)..' '..tostring(key)..' prepare')
    return true
end
...
local function release_protected_ring_slots(slots,reason)
    ...
    if reason then add_to_chat(158,'[WAR Ring] Releasing protected ring: '..reason) end
    if pause_swaps_active() then
        add_to_chat(158,'[WAR Ring] Release deferred until Pause ends.')
        return
    end
...
local function th_mark_tagged(target_id,source)
    if not target_id or not state or not state.TreasureMode then return end
    if state.TreasureMode.value~='Tag' or th_tracker.target_id~=target_id or th_tracker.tagged then return end
    th_tracker.tagged=true
    th_tracker.pending_target_id=nil
    add_to_chat(158,'[WAR TH] Target tagged'..(source and (' via '..source) or '')..'; restoring combat gear.')
    if not WAR_RUNTIME.unloading and not midaction() and type(handle_equipping_gear)=='function' and player then
...
local function reset_th_tracker(verbose)
    ...
    th_tracker.pending_target_id=nil
    th_sync_target()
    if verbose then add_to_chat(158,'[WAR TH] Tag state reset for current target.') end
end
...
local function try_echo_drops(generation)
    if WAR_RUNTIME.unloading or generation~=silence_echo.generation then return end
...
local berserk_sequence={pending=nil,generation=0}
...
local function movement_kiting_requested()
    return moving==true
end
...
local function check_moving()
    ...
    -- Native movement is the authority for automatic kiting.  Defense modes
    -- no longer suppress the ring: the movement overlay is applied after the
    -- selected PDT/MDT/MEVA set, while Doom and explicit gear freezes retain
...
local function queue_movement_gear_refresh()
    if WAR_RUNTIME.unloading or movement_monitor.refresh_queued then return end
    movement_monitor.refresh_queued=true
    send_command('gs c _movementrefresh '..WAR_RUNTIME.token)
end

local function native_movement_sample()
    if WAR_RUNTIME.unloading then return end
    local now=os.clock()
    ...
    -- The route now reports requested versus actually equipped.  Refreshing
    -- only when that label changes keeps the HUD truthful without redrawing
    -- it on every position sample.
    local route=movement_route_label()
    if route~=movement_monitor.last_route_label then
        movement_monitor.last_route_label=route
        if type(update_hud)=='function' then update_hud(false) end
    end
end
...
local function handle_war_action(act)
    if WAR_RUNTIME.unloading or not act then return end
    ...
    if (act.category==1 or act.category==3) and act.targets then
        for _,target in pairs(act.targets) do
            if target and target.id and target.id==th_tracker.target_id then
                for _,subaction in pairs(target.actions or {}) do
                    if (tonumber(subaction.param) or 0)>0 then
                        th_mark_tagged(target.id,act.category==1 and 'melee' or 'weapon skill')
                        return
                    end
                end
            end
...
local function register_war_events()
    unregister_war_events()
    if windower and type(windower.raw_register_event)=='function' then
        track_war_event(windower.raw_register_event('action',handle_war_action))
        track_war_event(windower.raw_register_event('prerender',native_movement_sample))
    end
    if windower and type(windower.register_event)=='function' then
        track_war_event(windower.register_event('zone change',function()
            if WAR_RUNTIME.unloading then return end
            clear_berserk_sequence()
...
function user_setup()
    ...
    send_command('bind f10 gs c toggle BerserkAuto')
    ...
    send_command('bind ^f3 gs c wardefense next')
    send_command('bind ^f4 gs c cycle WeaponskillMode')
    send_command('bind ^f7 gs c cycle IdleMode')
    send_command('bind ^f9 gs c warweapon previous')
    send_command('bind ^f10 gs c warweapon next')
    ...
    movement_monitor.last_route_label=nil
    ...
    set_macro_page(2,11)
    send_command('wait 4;gs c _startupkeys '..WAR_RUNTIME.token)
    send_command('wait 2;input /lockstyleset 40')
end

function user_unload()
    if save_war_hud_preferences then save_war_hud_preferences(true) end
    WAR_RUNTIME.unloading=true
    WAR_RUNTIME.token='unloaded-'..tostring(os.clock())
    ...
    for _,key in ipairs({'w','r','e','b','p','h','f','t','z','v'}) do send_command('unbind @'..key) end
    send_command('unbind ^@h')
    for _,key in ipairs({'^f1','^f2','^f3','^f4','^f7','!f1','!f2','!f3'}) do send_command('unbind '..key) end
...
function init_gear_sets()
    ...
    sets.precast.WS["King's Justice"].Acc=set_combine(sets.precast.WS.MultiHit.Acc,{})
    sets.precast.WS["King's Justice"].PDL=set_combine(sets.precast.WS.MultiHit.PDL,{})
...
local function apply_defense_control(requested,quiet)
    ...
    if not applying_playstyle and not swaps_frozen() and not midaction() and type(handle_equipping_gear)=='function' then
        handle_equipping_gear(player.status)
    end
    if type(update_hud)=='function' then update_hud(true) end
...
local function playstyle_profile_matches(key)
    ...
    for _,change in ipairs(profile.modes) do
        ...
        local value=st and (st.value~=nil and st.value or st.current) or nil
...
local function apply_playstyle(requested,quiet)
    ...
    applying_playstyle=false
    check_weaponset(true)
    if not swaps_frozen() and not midaction() and type(handle_equipping_gear)=='function' then handle_equipping_gear(player.status) end
    if type(update_hud)=='function' then update_hud(true) end
...
local function issue_target_weaponskill(ws_name,source,preserve_sc_context)
    ...
    local target=current_target_mob()
...
local function resolve_context_weaponskill()
    local eq=player and player.equipment or {}
    local range=item_name(eq.range)
    if WAR_CONTEXT_WS_BY_RANGE[range] then
        return WAR_CONTEXT_WS_BY_RANGE[range],range,item_name(eq.ammo)
    end
    local main=item_name(eq.main)
    return WAR_CONTEXT_WS_BY_MAIN[main],main,item_name(eq.sub)
end
...
function sc_cancel(reason)
    ...
    if had and not WAR_RUNTIME.unloading and type(update_hud)=='function' then update_hud(false) end
end
local function sc_note_resonance(prop,target_id,opener,actor_id)
    if not prop then return end
    local current=current_target_mob()
...
local function sc_refresh_choice()
    ...
    local sig=table.concat({tostring(sc_react.generation),tostring(item_name(eq.main) or ''),tostring(item_name(eq.sub) or ''),tostring(item_name(eq.range) or ''),ids and table.concat(ids,',') or 'unavailable'},'|')
...
function sc_opportunity_snapshot(now)
    ...
    if swaps_frozen() then out.status='SWAPS_FROZEN'; return out end
...
function execute_manual_skillchain()
    ...
    if not issued then manual_sc.pending=nil; return false end
    if type(update_hud)=='function' then update_hud(false) end
    coroutine.schedule(function()
        if WAR_RUNTIME.unloading then return end
...
local function fencer_rank_for_ws_set(ws_set,spell)
    ...
    for _,slot in ipairs(slots) do
        local piece=ws_set and item_name(ws_set[slot]) or nil
        if slot=='neck' and sets.precast.WS.FencerOverlay and sets.precast.WS.FencerOverlay.neck then
            piece=item_name(sets.precast.WS.FencerOverlay.neck)
        end
...
local function weapon_profile_tp_bonus()
    ...
    return meta and tonumber(meta.tp_bonus) or 0
end
...
local function effective_ws_tp(spell,include_moonshade)
    ...
    if include_moonshade then
        local e1,e2=item_name(ws_set.ear1),item_name(ws_set.ear2)
...
local function apply_max_tp_moonshade(spell)
    ...
    local ear1=item_name(ws_set.ear1)
    local ear2=item_name(ws_set.ear2)
...
function job_aftercast(spell,action,spellMap,eventArgs)
    ...
    if spell and not spell.interrupted and th_tracker.pending_target_id and
       spell.type~='WeaponSkill' and
       spell.target and spell.target.id==th_tracker.pending_target_id then
        th_mark_tagged(spell.target.id,spell.english)
    end
    check_weaponset(false)
    if movement_monitor.refresh_pending and not pause_swaps_active() then
        movement_monitor.refresh_pending=false
        check_moving()
    end
    if not pause_swaps_active() and not midaction()
...
local function selected_kiting_overlay(th_enabled)
    ...
    if th_enabled and slot=='ring2' and not protected_ring_active('ring1') then
...
function customize_melee_set(meleeSet)
    ...
    if state.HybridMode.value=='DT' then
        merge_melee_overlay(fencer and sets.engaged.FencerDTOverlay or sets.engaged.DTOverlay)
    end
    if dual_wield_melee_active() and sets.engaged.DWOverlay then
        merge_melee_overlay(sets.engaged.DWOverlay)
    end
    if state and state.Buff and state.Buff['Mighty Strikes'] and sets.buff.MightyStrikesTP then
        merge_melee_overlay(sets.buff.MightyStrikesTP)
    end
    local th_enabled=th_should_apply(th_sync_target())
    if th_enabled then merge_melee_overlay(sets.TreasureHunter) end
    if movement_kiting_requested() then merge_melee_overlay(selected_kiting_overlay(th_enabled)) end
    protected_ring_check()
    if next(melee_overlay) then return set_combine(meleeSet,melee_overlay) end
    return meleeSet
end

function customize_idle_set(idleSet)
    if movement_kiting_requested() then
...
function job_buff_change(buff,gain)
    ...
    if not swaps_frozen() and not midaction()
        and type(handle_equipping_gear)=='function' and player then
        handle_equipping_gear(player.status)
    end
    update_hud(false)
...
local function resume_swaps(message)
    ...
    if fishing_mode_active() then apply_fishing_slot_policy(); return end
    enable(unpack(ALL_EQUIP_SLOTS))
    invalidate_ring_lock_cache()
    movement_monitor.refresh_pending=false
    check_moving()
...
function job_state_change(descriptor,new_value,old_value)
    ...
    elseif descriptor=='FishingMode' or descriptor=='Fishing Mode' then
        if state.FishingMode.value then
            ...
            if pause_swaps_active() then
                ...
                apply_fishing_slot_policy()
                if moving and not midaction() and type(handle_equipping_gear)=='function' then
                    handle_equipping_gear(player.status)
                    apply_fishing_slot_policy()
                end
    ...
    elseif descriptor=='Treasure Hunter' or descriptor=='TreasureMode' then
        reset_th_tracker(false)
        add_to_chat(158,'[WAR TH] Mode: '..tostring(state.TreasureMode.value))
    ...
    if pause_swaps_active() then disable(unpack(ALL_EQUIP_SLOTS)); update_hud(false); return end
    if fishing_mode_active() then apply_fishing_slot_policy(); update_hud(false); return end
...
function job_sub_job_change(newSubjob,oldSubjob)
    ...
    if not swaps_frozen() and type(handle_equipping_gear)=='function' and player then
        handle_equipping_gear(player.status)
    end
    update_hud(true)
end
...
local function queue_war_hud_save()
    ...
    local function settle()
        ...
        coroutine.schedule(function()
            if WAR_RUNTIME.unloading then war_hud_drag_pending=false; return end
...
function update_hud(force)
    ...
    local th=state and state.TreasureMode and state.TreasureMode.value or 'Off'
    ...
    local warnings={}; if pause_swaps_active() then warnings[#warnings+1]='PAUSED' end; if fishing_mode_active() then warnings[#warnings+1]='FISHING' end; if buffactive and buffactive.doom then warnings[#warnings+1]='DOOM' end; if buffactive and buffactive.silence then warnings[#warnings+1]='SILENCED' end; if not weapon_ok then warnings[#warnings+1]='WEAPON MISMATCH' end
    ...
    if section_on('utility') then
        rows[#rows+1]=hrow('UTILITY',hfield('TH',th,th~='Off' and 'yellow' or 'dim')..'  '..hfield('Berserk Auto',berserk_auto and 'ON' or 'off',berserk_auto and 'green' or 'dim')..'  '..hfield('Move',movement_route_label(),moving and 'green' or 'dim'))
...
local function report_th()
    ...
    add_to_chat(158,string.format('[WAR TH] mode=%s | target=%s | tagged=%s',
...
function job_self_command(cmdParams,eventArgs)
    ...
    elseif cmd=='_movementrefresh' then
        ...
        movement_monitor.refresh_queued=false
        eventArgs.handled=true
        if WAR_RUNTIME.unloading or cmdParams[2]~=WAR_RUNTIME.token then return end
        if not movement_monitor.refresh_pending then
            update_hud(false)
            return
        end
        check_moving()
        if pause_swaps_active() or (type(midaction)=='function' and midaction()) then
            return
        end

        local slot=movement_kiting_requested() and movement_ring_slot() or nil
        if slot and not protected_ring_active(slot) then
    ...
    elseif cmd=='_startupkeys' then
        if cmdParams[2]==WAR_RUNTIME.token and not WAR_RUNTIME.unloading then report_keybinds(true) end
        eventArgs.handled=true
    ...
    elseif cmd=='ringinfo' then
        add_to_chat(158,'[WAR Rings] ring1='..tostring(current_ring_name('ring1'))..
            ' protected='..tostring(NO_SWAP_GEAR[current_ring_name('ring1')]==true))
        add_to_chat(158,'[WAR Rings] ring2='..tostring(current_ring_name('ring2'))..
            ' protected='..tostring(NO_SWAP_GEAR[current_ring_name('ring2')]==true))
        eventArgs.handled=true
    ...
    elseif cmd=='moveinfo' then
        add_to_chat(158,string.format(
            '[WAR Move] moving=%s | Auto_Kite=%s | manual=%s | route=%s | pending=%s | queued=%s | defense=%s | sample=%.2fs | stop debounce=%.2fs',
            tostring(moving),tostring(state.Auto_Kite.value),tostring(state.Kiting.value),
            movement_route_label(),tostring(movement_monitor.refresh_pending),
            tostring(movement_monitor.refresh_queued),tostring(state.DefenseMode.value),
            movement_monitor.sample_interval,movement_monitor.stop_debounce))
        add_to_chat(158,'[WAR Move] ring1='..tostring(current_ring_name('ring1'))..
            ' lock='..tostring(ring_lock_state.ring1)..
            ' | ring2='..tostring(current_ring_name('ring2'))..
            ' lock='..tostring(ring_lock_state.ring2))
        eventArgs.handled=true
...
function display_current_job_state(eventArgs)
    local msg=string.format(
        ...
        tostring(state.IdleMode.value),tostring(state.TreasureMode.value),
        moving and 'ON' or 'off',state.BerserkAuto.value and 'ON' or 'OFF',
